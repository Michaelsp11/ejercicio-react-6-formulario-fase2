...
export const Paso3 = (props) => {
  const { datosAcceso, avanzaPaso, retrocedePaso, guardarDatos } = props;
  const { datos, setDato } = useFormulario(datosAcceso);
  const { username, password, recordarPassword } = datos;
  const enviaPaso = (e) => {
    e.preventDefault();
    guardarDatos(datos);
    avanzaPaso();
