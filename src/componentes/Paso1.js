...
export const Paso1 = (props) => {
  const { datosPersonales, avanzaPaso, guardarDatos } = props;
  const { datos, setDato } = useFormulario(datosPersonales);
  const { nombre, apellidos, fechaNacimiento, email } = datos;
  const enviaPaso = (e) => {
    e.preventDefault();
    guardarDatos(datos);
    avanzaPaso();
