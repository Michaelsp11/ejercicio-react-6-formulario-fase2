...
export const Paso2 = (props) => {
  const { datosRegistro, avanzaPaso, retrocedePaso, guardarDatos } = props;
  const { datos, setDato } = useFormulario(datosRegistro);
  const { username, password, repitePassword } = datos;
  const enviaPaso = (e) => {
    e.preventDefault();
    guardarDatos(datos);
    avanzaPaso();
